...
{
    gsx_metal_backend_buffer *x_metal_buffer = NULL;
    gsx_metal_backend_buffer *out_metal_buffer = NULL;
    gsx_metal_backend *metal_backend = NULL;
    ...
    gsx_size_t growth_offset_bytes = 0;
    gsx_size_t growth_size_bytes = 0;
    gsx_error error = { GSX_ERROR_SUCCESS, NULL };
    ...
    rows_to_copy = x_shape[0] < out_shape[0] ? (gsx_size_t)x_shape[0] : (gsx_size_t)out_shape[0];
    if(gsx_size_mul_overflows(rows_to_copy, out_row_bytes, &copy_size_bytes)) {
        return gsx_make_error(GSX_ERROR_OUT_OF_RANGE, "resize copy size overflows");
    }

    x_metal_buffer = gsx_metal_backend_buffer_from_base(x_view->buffer);
    out_metal_buffer = gsx_metal_backend_buffer_from_base(dst_buffer);
    metal_backend = gsx_metal_backend_from_base(dst_buffer->buffer_type->backend);

    if(gsx_metal_backend_buffer_is_cpu_visible(x_metal_buffer) && gsx_metal_backend_buffer_is_cpu_visible(out_metal_buffer)) {
        x_bytes = gsx_metal_backend_tensor_data(x_metal_buffer, x_view, 0);
        out_bytes = gsx_metal_backend_tensor_data(out_metal_buffer, out_view, 0);
        if(copy_size_bytes != 0) {
            memmove(out_bytes, x_bytes, (size_t)copy_size_bytes);
        }
        if(out_shape[0] > (gsx_index_t)rows_to_copy) {
            if(gsx_size_mul_overflows(rows_to_copy, out_row_bytes, &growth_offset_bytes)) {
                return gsx_make_error(GSX_ERROR_OUT_OF_RANGE, "resize growth offset overflows");
            }
            growth_size_bytes = out_view->size_bytes - growth_offset_bytes;
            memset(out_bytes + (size_t)growth_offset_bytes, 0, (size_t)growth_size_bytes);
        }
        return gsx_make_error(GSX_ERROR_SUCCESS, NULL);
    }

    if(copy_size_bytes != 0) {
        error = gsx_metal_backend_commit_copy(
            metal_backend,
            (id<MTLBuffer>)x_metal_buffer->mtl_buffer,
            x_view->offset_bytes,
            (id<MTLBuffer>)out_metal_buffer->mtl_buffer,
            out_view->offset_bytes,
            copy_size_bytes
        );
        if(!gsx_error_is_success(error)) {
            return error;
    ...
    if(out_shape[0] > (gsx_index_t)rows_to_copy) {
        ...
        return gsx_metal_backend_commit_fill(
            metal_backend,
            (id<MTLBuffer>)out_metal_buffer->mtl_buffer,
            out_view->offset_bytes + growth_offset_bytes,
            growth_size_bytes,
            0
        );
    }
