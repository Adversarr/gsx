...
struct tensor_plan_payload {
    const gsx_tensor_desc *descs;
    gsx_index_t tensor_count;
};

static gsx_error plan_tensor_desc_list(gsx_arena_t dry_run_arena, void *user_data)
{
    const tensor_plan_payload *payload = static_cast<const tensor_plan_payload *>(user_data);
    std::vector<gsx_tensor_t> tensors;
    gsx_error error{};

    if(dry_run_arena == nullptr || payload == nullptr) {
        return gsx_error{ GSX_ERROR_INVALID_ARGUMENT, "dry_run_arena and payload must be non-null" };
    }

    tensors.resize(static_cast<std::size_t>(payload->tensor_count), nullptr);
    error = gsx_tensor_init_many(tensors.data(), dry_run_arena, payload->descs, payload->tensor_count);
    ...
    gsx_size_t requested_alignment_bytes = 0)
{
    ...
    desc.arena_desc.requested_alignment_bytes = requested_alignment_bytes;
    desc.arena_desc.dry_run = true;
    desc.count = count;
    desc.aux_flags = aux_flags;
    error = gsx_gs_init(&gs, &desc);
    if(!gsx_error_is_success(error)) {
    ...
    error = gsx_arena_get_required_bytes(info.arena, &required_bytes);
    if(!gsx_error_is_success(error)) {
        ADD_FAILURE() << (error.message != nullptr ? error.message : "");
        (void)gsx_gs_free(gs);
        return 0;
    }
    error = gsx_gs_free(gs);
    if(!gsx_error_is_success(error)) {
        ADD_FAILURE() << (error.message != nullptr ? error.message : "");
        return 0;
    }
...
static gsx_size_t round_buffer_type_capacity(gsx_backend_buffer_type_t buffer_type, gsx_size_t requested_bytes)
{
    gsx_size_t rounded_bytes = 0;
    gsx_error error = gsx_backend_buffer_type_get_alloc_size(buffer_type, requested_bytes, &rounded_bytes);
...
TEST(CoreRuntime, TensorInitManyAndFreeManyHandleMixedLayouts)
{
    gsx_backend_t backend = create_cpu_backend();
    gsx_backend_buffer_type_t buffer_type = find_buffer_type(backend, GSX_BACKEND_BUFFER_TYPE_DEVICE);
    gsx_arena_t arena = nullptr;
    gsx_arena_desc arena_desc{};
    std::array<gsx_tensor_desc, 3> descs{};
    std::array<gsx_tensor_t, 3> tensors{};
    gsx_tensor_info info_a{};
    gsx_tensor_info info_b{};
    gsx_tensor_info info_c{};

    arena_desc.initial_capacity_bytes = 1024;
    ASSERT_GSX_SUCCESS(gsx_arena_init(&arena, buffer_type, &arena_desc));

    descs[0] = make_f32_tensor_desc(nullptr, 8, 64);
    descs[1] = make_rank1_tensor_desc(nullptr, 5, GSX_DATA_TYPE_I32);
    descs[1].requested_alignment_bytes = 16;
    descs[2] = make_f32_tensor_desc_with_shape(nullptr, { 2, 3, 4, 0 }, 3, 32);

    ASSERT_GSX_SUCCESS(gsx_tensor_init_many(tensors.data(), arena, descs.data(), (gsx_index_t)tensors.size()));
    ASSERT_GSX_SUCCESS(gsx_tensor_get_info(tensors[0], &info_a));
    ASSERT_GSX_SUCCESS(gsx_tensor_get_info(tensors[1], &info_b));
    ASSERT_GSX_SUCCESS(gsx_tensor_get_info(tensors[2], &info_c));
    EXPECT_EQ(info_a.size_bytes, 8U * sizeof(float));
    EXPECT_EQ(info_b.size_bytes, 5U * sizeof(std::int32_t));
    EXPECT_EQ(info_c.size_bytes, 24U * sizeof(float));
    EXPECT_GE(info_a.effective_alignment_bytes, 64U);
    EXPECT_GE(info_b.effective_alignment_bytes, 16U);
    EXPECT_GE(info_c.effective_alignment_bytes, 32U);

    ASSERT_GSX_SUCCESS(gsx_tensor_free_many(tensors.data(), (gsx_index_t)tensors.size()));
    EXPECT_EQ(tensors[0], nullptr);
    EXPECT_EQ(tensors[1], nullptr);
    EXPECT_EQ(tensors[2], nullptr);
    ASSERT_GSX_SUCCESS(gsx_arena_free(arena));
    ASSERT_GSX_SUCCESS(gsx_backend_free(backend));
}

TEST(CoreRuntime, TensorInitManyRollsBackOnFailure)
{
    gsx_backend_t backend = create_cpu_backend();
    gsx_backend_buffer_type_t buffer_type = find_buffer_type(backend, GSX_BACKEND_BUFFER_TYPE_DEVICE);
    gsx_arena_t arena = nullptr;
    gsx_arena_desc arena_desc{};
    std::array<gsx_tensor_desc, 2> descs{};
    std::array<gsx_tensor_t, 2> tensors{};
    gsx_arena_info arena_info{};

    arena_desc.initial_capacity_bytes = 128;
    ASSERT_GSX_SUCCESS(gsx_arena_init(&arena, buffer_type, &arena_desc));

    descs[0] = make_f32_tensor_desc(nullptr, 8);
    descs[1] = make_f32_tensor_desc(nullptr, 80);

    EXPECT_GSX_CODE(gsx_tensor_init_many(tensors.data(), arena, descs.data(), (gsx_index_t)tensors.size()), GSX_ERROR_OUT_OF_RANGE);
    ASSERT_GSX_SUCCESS(gsx_arena_get_info(arena, &arena_info));
    EXPECT_EQ(arena_info.active_tensor_count, 0U);
    EXPECT_EQ(arena_info.used_bytes, 0U);
    EXPECT_EQ(tensors[0], nullptr);
    EXPECT_EQ(tensors[1], nullptr);
...
TEST(CoreRuntime, TensorPlanRequiredBytesMatchesManualDryRunAndArenaPlanner)
{
    gsx_backend_t backend = create_cpu_backend();
    gsx_backend_buffer_type_t buffer_type = find_buffer_type(backend, GSX_BACKEND_BUFFER_TYPE_DEVICE);
    gsx_arena_t dry_arena = nullptr;
    gsx_arena_desc arena_desc{};
    std::array<gsx_tensor_desc, 3> descs{};
    std::array<gsx_tensor_t, 3> manual_tensors{};
    gsx_size_t manual_required_bytes = 0;
    gsx_size_t tensor_plan_required_bytes_value = 0;
    gsx_size_t callback_plan_required_bytes = 0;
    tensor_plan_payload payload{};

    arena_desc.initial_capacity_bytes = 0;
    arena_desc.requested_alignment_bytes = 128;
    arena_desc.dry_run = true;

    descs[0] = make_f32_tensor_desc(nullptr, 13, 32);
    descs[1] = make_rank1_tensor_desc(nullptr, 9, GSX_DATA_TYPE_I32);
    descs[1].requested_alignment_bytes = 64;
    descs[2] = make_f32_tensor_desc_with_shape(nullptr, { 3, 5, 7, 0 }, 3, 256);

    ASSERT_GSX_SUCCESS(gsx_arena_init(&dry_arena, buffer_type, &arena_desc));
    ASSERT_GSX_SUCCESS(gsx_tensor_init_many(manual_tensors.data(), dry_arena, descs.data(), (gsx_index_t)manual_tensors.size()));
    ASSERT_GSX_SUCCESS(gsx_arena_get_required_bytes(dry_arena, &manual_required_bytes));
    ASSERT_GSX_SUCCESS(gsx_tensor_free_many(manual_tensors.data(), (gsx_index_t)manual_tensors.size()));
    ASSERT_GSX_SUCCESS(gsx_arena_free(dry_arena));

    ASSERT_GSX_SUCCESS(gsx_tensor_plan_required_bytes(
        buffer_type,
        &arena_desc,
        descs.data(),
        (gsx_index_t)descs.size(),
        &tensor_plan_required_bytes_value));

    payload.descs = descs.data();
    payload.tensor_count = (gsx_index_t)descs.size();
    ASSERT_GSX_SUCCESS(gsx_arena_plan_required_bytes(
        buffer_type,
        &arena_desc,
        plan_tensor_desc_list,
        &payload,
        &callback_plan_required_bytes));
...
TEST(CoreRuntime, EmptyRealArenaOverflowGrowsAndReportsRequiredBytes)
{
    gsx_backend_t backend = create_cpu_backend();
    gsx_backend_buffer_type_t buffer_type = find_buffer_type(backend, GSX_BACKEND_BUFFER_TYPE_DEVICE);
    gsx_arena_t arena = nullptr;
...
TEST(CoreRuntime, DryRunArenaOverflowGrowsAndReportsRequiredBytes)
{
    gsx_backend_t backend = create_cpu_backend();
    gsx_backend_buffer_type_t buffer_type = find_buffer_type(backend, GSX_BACKEND_BUFFER_TYPE_DEVICE);
    gsx_arena_t arena = nullptr;
...
TEST(CoreRuntime, RealArenaGrowsOnlyWithoutLiveTensors)
{
    gsx_backend_t backend = create_cpu_backend();
    gsx_backend_buffer_type_t buffer_type = find_buffer_type(backend, GSX_BACKEND_BUFFER_TYPE_DEVICE);
    gsx_arena_t arena = nullptr;
...
TEST(CoreRuntime, TensorFreeDropsLiveStatsButDoesNotReclaimCursor)
{
    ...
    ASSERT_GSX_SUCCESS(gsx_tensor_init(&tensor_b, &tensor_desc_b));
    ASSERT_GSX_SUCCESS(gsx_arena_get_required_bytes(arena, &required_bytes));
...
TEST(CoreRuntime, TensorReduceDryRunAllOpsKeepBytesUnchangedWithoutMutatingOutputOnCpu)
{
    gsx_backend_t backend = create_cpu_backend();
    gsx_backend_buffer_type_t buffer_type = find_buffer_type(backend, GSX_BACKEND_BUFFER_TYPE_DEVICE);
    gsx_arena_t arena = nullptr;
    ...
    for(std::size_t i = 0; i < x_values.size(); ++i) {
        x_values[i] = static_cast<float>(i + 1);
        target_values[i] = static_cast<float>((i + 5) * 0.5f);
    }
    x_shape[0] = 2;
    x_shape[1] = 3;
    x_shape[2] = 4;
...
TEST(CoreRuntime, GsLifecycleAndFieldQueryRespectAuxFlags)
{
    ...
    ASSERT_GSX_SUCCESS(gsx_arena_get_info(gs_info.arena, &arena_info));
    EXPECT_NE(gs_info.arena, nullptr);
    EXPECT_EQ(gs_info.count, 3U);
    EXPECT_EQ(gs_info.aux_flags, GSX_GS_AUX_GRAD_ACC);
    required_bytes = plan_gs_required_bytes(buffer_type, 3, GSX_GS_AUX_GRAD_ACC);
    rounded_bytes = round_buffer_type_capacity(buffer_type, required_bytes);
    EXPECT_EQ(arena_info.capacity_bytes, rounded_bytes);
...
TEST(CoreRuntime, GsSetFieldZeroGradientsAndClampOpacityWork)
{
    gsx_backend_t backend = create_cpu_backend();
    gsx_backend_buffer_type_t buffer_type = find_buffer_type(backend, GSX_BACKEND_BUFFER_TYPE_DEVICE);
    gsx_arena_t replacement_arena = nullptr;
    gsx_arena_desc replacement_arena_desc{};
    gsx_gs_t gs = nullptr;
    gsx_gs_desc gs_desc{};
    ...
    replacement_arena_desc.initial_capacity_bytes = 1024;
    ASSERT_GSX_SUCCESS(gsx_arena_init(&replacement_arena, buffer_type, &replacement_arena_desc));
    replacement_desc = make_f32_tensor_desc_with_shape(replacement_arena, { 2, 3, 0, 0 }, 2);
    ...
    ASSERT_GSX_SUCCESS(gsx_arena_free(replacement_arena));
    ASSERT_GSX_SUCCESS(gsx_gs_free(gs));
    ASSERT_GSX_SUCCESS(gsx_backend_free(backend));
}
...
TEST(CoreRuntime, GsAuxMutationsStructuralOpsAndFiniteCheckWork)
{
    ...
    control_arena_desc.initial_capacity_bytes = 1024;
    ...
    ASSERT_GSX_SUCCESS(gsx_gs_get_info(gs, &gs_info));
    ASSERT_GSX_SUCCESS(gsx_arena_get_info(gs_info.arena, &arena_info));
    EXPECT_NE(gs_info.arena, arena_before);
    EXPECT_EQ(gs_info.aux_flags, GSX_GS_AUX_GRAD_ACC | GSX_GS_AUX_VISIBLE_COUNTER);
    required_bytes = plan_gs_required_bytes(buffer_type, 3, gs_info.aux_flags);
    EXPECT_EQ(arena_info.capacity_bytes, round_buffer_type_capacity(buffer_type, required_bytes));
    ...
    permutation_desc = make_rank1_tensor_desc(control_arena, 3, GSX_DATA_TYPE_I32);
    gather_index_desc = make_rank1_tensor_desc(control_arena, 2, GSX_DATA_TYPE_I32);
    ...
    required_bytes = plan_gs_required_bytes(buffer_type, 2, gs_info.aux_flags);
    EXPECT_EQ(arena_info.capacity_bytes, round_buffer_type_capacity(buffer_type, required_bytes));
    ASSERT_GSX_SUCCESS(gsx_gs_get_field(gs, GSX_GS_FIELD_OPACITY, &opacity));
...
TEST(CoreRuntime, GsGatherToLargerCountRebuildsExactArenaAndPreservesData)
{
    gsx_backend_t backend = create_cpu_backend();
    gsx_backend_buffer_type_t buffer_type = find_buffer_type(backend, GSX_BACKEND_BUFFER_TYPE_DEVICE);
    gsx_gs_desc gs_desc{};
    ...
    gsx_arena_desc index_arena_desc{};
    gsx_tensor_t index = nullptr;
    gsx_tensor_t opacity = nullptr;
    gsx_tensor_desc index_desc{};
    gsx_arena_t arena_before = nullptr;
    std::array<int32_t, 8> gather_values = { 0, 1, 2, 3, 0, 1, 2, 3 };
    std::array<float, 4> opacity_values = { 10.0f, 20.0f, 30.0f, 40.0f };
    std::array<float, 8> roundtrip = {};
    gsx_size_t required_bytes = 0;
    ...
    index_arena_desc.initial_capacity_bytes = 1024;
    ASSERT_GSX_SUCCESS(gsx_arena_init(&index_arena, buffer_type, &index_arena_desc));
    ...
    ASSERT_GSX_SUCCESS(gsx_gs_gather(gs, index));
    ASSERT_GSX_SUCCESS(gsx_gs_get_info(gs, &gs_info));
    ASSERT_GSX_SUCCESS(gsx_arena_get_info(gs_info.arena, &arena_info));
    EXPECT_NE(gs_info.arena, arena_before);
    EXPECT_EQ(gs_info.count, 8U);
    required_bytes = plan_gs_required_bytes(buffer_type, 8, GSX_GS_AUX_NONE);
    EXPECT_EQ(arena_info.capacity_bytes, round_buffer_type_capacity(buffer_type, required_bytes));
    ASSERT_GSX_SUCCESS(gsx_gs_get_field(gs, GSX_GS_FIELD_OPACITY, &opacity));
    ASSERT_GSX_SUCCESS(gsx_tensor_download(opacity, roundtrip.data(), sizeof(roundtrip)));
    EXPECT_FLOAT_EQ(roundtrip[0], 10.0f);
    EXPECT_FLOAT_EQ(roundtrip[1], 20.0f);
    EXPECT_FLOAT_EQ(roundtrip[2], 30.0f);
    EXPECT_FLOAT_EQ(roundtrip[3], 40.0f);
    EXPECT_FLOAT_EQ(roundtrip[4], 10.0f);
    EXPECT_FLOAT_EQ(roundtrip[5], 20.0f);
    EXPECT_FLOAT_EQ(roundtrip[6], 30.0f);
    EXPECT_FLOAT_EQ(roundtrip[7], 40.0f);
...
TEST(CoreRuntime, GsSetAuxEnabledRebuildsFromMinimalLayout)
{
    gsx_backend_t backend = create_cpu_backend();
    gsx_backend_buffer_type_t buffer_type = find_buffer_type(backend, GSX_BACKEND_BUFFER_TYPE_DEVICE);
    gsx_gs_t gs = nullptr;
    ...
    gsx_tensor_t opacity = nullptr;
    gsx_tensor_info sh1_info{};
    gsx_arena_t arena_before = nullptr;
    gsx_size_t required_bytes = 0;
    std::array<float, 64> opacity_values = {};
    std::array<float, 64> roundtrip = {};

    for(std::size_t i = 0; i < opacity_values.size(); ++i) {
        opacity_values[i] = static_cast<float>(i + 1);
    }
    ...
    required_bytes = plan_gs_required_bytes(buffer_type, 64, GSX_GS_AUX_SH1);
    EXPECT_EQ(arena_info.capacity_bytes, round_buffer_type_capacity(buffer_type, required_bytes));
    ASSERT_GSX_SUCCESS(gsx_gs_get_field(gs, GSX_GS_FIELD_SH1, &sh1));
    ASSERT_GSX_SUCCESS(gsx_gs_get_field(gs, GSX_GS_FIELD_OPACITY, &opacity));
    ASSERT_GSX_SUCCESS(gsx_tensor_download(opacity, roundtrip.data(), sizeof(roundtrip)));
    EXPECT_EQ(roundtrip, opacity_values);
    ASSERT_GSX_SUCCESS(gsx_tensor_get_info(sh1, &sh1_info));
    std::vector<float> sh1_values(sh1_info.size_bytes / sizeof(float), 1.0f);
    ASSERT_GSX_SUCCESS(gsx_tensor_download(sh1, sh1_values.data(), sh1_info.size_bytes));
    for(float value : sh1_values) {
...
TEST(CoreRuntime, GsResizeRebuildsFromMinimalLayoutAndPreservesData)
{
    gsx_backend_t backend = create_cpu_backend();
    gsx_backend_buffer_type_t buffer_type = find_buffer_type(backend, GSX_BACKEND_BUFFER_TYPE_DEVICE);
    gsx_gs_t gs = nullptr;
    ...
    ASSERT_GSX_SUCCESS(gsx_gs_resize(gs, 8));
    ASSERT_GSX_SUCCESS(gsx_gs_get_info(gs, &gs_info));
    ASSERT_GSX_SUCCESS(gsx_arena_get_info(gs_info.arena, &arena_info));
    EXPECT_NE(gs_info.arena, arena_before);
    EXPECT_EQ(gs_info.count, 8U);
    ...
    EXPECT_FLOAT_EQ(roundtrip[2], 30.0f);
    EXPECT_FLOAT_EQ(roundtrip[3], 40.0f);
    EXPECT_FLOAT_EQ(roundtrip[4], 0.0f);
    EXPECT_FLOAT_EQ(roundtrip[5], 0.0f);
