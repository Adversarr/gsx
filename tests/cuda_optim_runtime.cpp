...
static void adam_apply_group_clip(AdamRefGroup *group)
{
...

    {
        ...
        float scale = (float)((double)group->max_grad_norm / norm);
        for(float &value : group->grads) {
            value *= scale;
        }
    }
...
class CudaOptimRuntimeTest : public ::testing::Test {
protected:
    void SetUp() override
    {
        if(!has_cuda_device()) {
...
TEST_F(CudaOptimRuntimeTest, InitRejectsHostPinnedStateBufferType)
{
...
TEST_F(CudaOptimRuntimeTest, StepSelectionResetAndLearningRateMatchReferenceAdam)
{
    gsx_backend_t backend = create_cuda_backend();
    gsx_backend_buffer_type_t device_buffer_type = find_buffer_type(backend, GSX_BACKEND_BUFFER_TYPE_DEVICE);
...
TEST_F(CudaOptimRuntimeTest, ClipMatchesReferenceAdam)
{
    gsx_backend_t backend = create_cuda_backend();
    gsx_backend_buffer_type_t device_buffer_type = find_buffer_type(backend, GSX_BACKEND_BUFFER_TYPE_DEVICE);
...
TEST_F(CudaOptimRuntimeTest, PermuteRejectsInvalidControlAndKeepsAdamState)
{
    gsx_backend_t backend = create_cuda_backend();
    gsx_backend_buffer_type_t device_buffer_type = find_buffer_type(backend, GSX_BACKEND_BUFFER_TYPE_DEVICE);
    ManualTensor parameter = make_rank1_tensor<float>(device_buffer_type, GSX_DATA_TYPE_F32, { 1.0f, 2.0f, 3.0f });
...
TEST_F(CudaOptimRuntimeTest, ConsecutivePermutesReuseScratchAndFreeAfterCommit)
{
    gsx_backend_t backend = create_cuda_backend();
    gsx_backend_buffer_type_t device_buffer_type = find_buffer_type(backend, GSX_BACKEND_BUFFER_TYPE_DEVICE);
    ManualTensor parameter = make_rank1_tensor<float>(device_buffer_type, GSX_DATA_TYPE_F32, { 1.0f, 2.0f, 3.0f });
...
TEST_F(CudaOptimRuntimeTest, HandleRemainsValidAfterMutationValidationFailures)
{
    gsx_backend_t backend = create_cuda_backend();
    gsx_backend_buffer_type_t device_buffer_type = find_buffer_type(backend, GSX_BACKEND_BUFFER_TYPE_DEVICE);
    ManualTensor parameter = make_rank1_tensor<float>(device_buffer_type, GSX_DATA_TYPE_F32, { 1.0f, 2.0f, 3.0f });
