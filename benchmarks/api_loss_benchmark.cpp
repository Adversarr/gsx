...
static bool gsx_init_cuda_reduce_unary_context(
    ...
    std::string *out_error)
{
    ...
    if(!gsx_error_ok(gsx_get_backend_device_by_type(GSX_BACKEND_TYPE_CUDA, 0, &backend_device)) || backend_device == nullptr) {
        *out_error = "CUDA backend device unavailable";
        return false;
    }
    ...
    ctx.outer_count = (gsx_size_t)outer_count;
    ctx.reduce_count = (gsx_size_t)reduce_count;
    ctx.element_count = element_count;
    *out_ctx = ctx;
    return true;
}

static bool gsx_init_cuda_reduce_binary_context(
...
static void bm_reduce_unary_cuda(benchmark::State &state, BenchmarkUnaryReduceOp op)
{
    ReduceUnaryBenchmarkContext ctx = {};
    std::string error;
    gsx_index_t outer_count = (gsx_index_t)state.range(0);
    gsx_index_t reduce_count = (gsx_index_t)state.range(1);

    if(!gsx_init_cuda_reduce_unary_context(&ctx, outer_count, reduce_count, &error)) {
        state.SkipWithError(error.c_str());
        return;
    }
    ...
    for(auto _ : state) {
        ...
        if(!gsx_error_ok(gsx_tensor_set_zero(ctx.output))) {
            state.SkipWithError("CUDA unary reduce output reset failed");
            break;
        }
        switch(op) {
        ...
        if(!gsx_error_ok(op_error)) {
            state.SkipWithError("CUDA unary reduce failed");
            break;
        }
        if(!gsx_sync_backend_if_cuda(ctx.backend, &error)) {
            state.SkipWithError(error.c_str());
...
static void BM_Core_ReduceCudaSum(benchmark::State &state)
{
    bm_reduce_unary_cuda(state, BenchmarkUnaryReduceOp::Sum);
}

static void BM_Core_ReduceCudaMean(benchmark::State &state)
{
    bm_reduce_unary_cuda(state, BenchmarkUnaryReduceOp::Mean);
}

static void BM_Core_ReduceCudaMax(benchmark::State &state)
{
    bm_reduce_unary_cuda(state, BenchmarkUnaryReduceOp::Max);
}

static void bm_reduce_binary_cuda(benchmark::State &state, BenchmarkBinaryReduceOp op)
{
    ReduceBinaryBenchmarkContext ctx = {};
    std::string error;
    gsx_index_t outer_count = (gsx_index_t)state.range(0);
    ...
    if(!gsx_init_cuda_reduce_binary_context(&ctx, outer_count, reduce_count, &error)) {
        state.SkipWithError(error.c_str());
        return;
    }
    ...
    for(auto _ : state) {
        ...
        if(!gsx_error_ok(gsx_tensor_set_zero(ctx.output))) {
            state.SkipWithError("CUDA binary reduce output reset failed");
            break;
        }
        switch(op) {
        ...
        if(!gsx_error_ok(op_error)) {
            state.SkipWithError("CUDA binary reduce failed");
            break;
        }
        if(!gsx_sync_backend_if_cuda(ctx.backend, &error)) {
            state.SkipWithError(error.c_str());
...
static void BM_Core_ReduceCudaMse(benchmark::State &state)
{
    bm_reduce_binary_cuda(state, BenchmarkBinaryReduceOp::Mse);
}

static void BM_Core_ReduceCudaMae(benchmark::State &state)
{
    bm_reduce_binary_cuda(state, BenchmarkBinaryReduceOp::Mae);
}

BENCHMARK(BM_Loss_SsimCpu)->Args({ 128, 128 });
BENCHMARK(BM_Loss_SsimCuda)->Args({ 512, 512 });
BENCHMARK(BM_Core_ReduceCudaSum)->Args({ 4096, 256 })->Args({ 1024, 4096 });
BENCHMARK(BM_Core_ReduceCudaMean)->Args({ 4096, 256 })->Args({ 1024, 4096 });
BENCHMARK(BM_Core_ReduceCudaMax)->Args({ 4096, 256 })->Args({ 1024, 4096 });
BENCHMARK(BM_Core_ReduceCudaMse)->Args({ 4096, 256 })->Args({ 1024, 4096 });
BENCHMARK(BM_Core_ReduceCudaMae)->Args({ 4096, 256 })->Args({ 1024, 4096 });
